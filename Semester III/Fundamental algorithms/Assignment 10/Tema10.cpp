...
int get_neighbors(const Grid *grid, Point p, Point neighb[])
{
    ...
    int num_neighbors = 0;
    //sus
    if (p.row > 0 && grid->mat[p.row - 1][p.col] == 0)//daca nu e perete
    {
        Point n;
        n.row = p.row - 1;
        n.col = p.col;
        neighb[num_neighbors++] = n;
    }
    //jos
    ...
    //dreapta
    if (p.col < grid->cols && grid->mat[p.row][p.col + 1] == 0)
    {
        Point n;
        n.row = p.row;
        n.col = p.col + 1;
...
void print_bfs_tree(Graph *graph)
{
    ...
    if(!err){
        ...
        // you can adapt the code for transforming and printing multi-way trees
